...
case object ConnectStructure extends PacketStructure[Connect] {
  ...
  private val willQoS = (p: Connect) => p.willMessage.fold(zero :: zero)(qosStructure of _.qos) //3.1.2.6
  ...
  private val willPayload = (p: Connect) => p.willMessage.map(_.payload).fold(empty)(bytesStructure of _) //3.1.3.3
  private val username = (p: Connect) => p.credential.map(_.username) //3.1.3.4
  private val password = (p: Connect) => p.credential.flatMap(_.password).fold(empty)(bytesStructure of _) //3.1.3.5
