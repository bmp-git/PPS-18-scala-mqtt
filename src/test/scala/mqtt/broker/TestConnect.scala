...
class TestConnect(ConnectPacketHandler: (State, Connect, Socket) => State) extends FunSuite {
  test("Sending a connect packet with and unsupported protocol name should disconnect") {
    ...
    assert(bs1.closing.get(sample_socket_0).isDefined)
  }
  ...
  test("Sending a legit connect packet should respond with ack 0") {
    val bs1 = ConnectPacketHandler(bs0, sample_connect_packet_0, sample_socket_0)
  ...
  test("A Connect from a new socket should disconnect the old one") {
    val bs1 = ConnectPacketHandler(bs0, sample_connect_packet_0, sample_socket_0)
    val bs2 = ConnectPacketHandler(bs1, sample_connect_packet_0, sample_socket_1)
  ...
  test("Connecting two times should disconnect") {
    val bs1 = ConnectPacketHandler(bs0, sample_connect_packet_0, sample_socket_0)
    val bs2 = ConnectPacketHandler(bs1, sample_connect_packet_0, sample_socket_0)
    assert(bs2.closing.get(sample_socket_0).isDefined)
  }
